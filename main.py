import sys
import random
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QPushButton, QGraphicsRectItem, QGraphicsScene, QGraphicsView
from PyQt5.QtCore import Qt, QTimer

class MovingSquare(QGraphicsRectItem):
    def __init__(self, x, y):
        super().__init__(x, y, 50, 50)
        self.setBrush(Qt.red)  # Цвет квадрата

    def move(self):
        self.setPos(self.x() + random.randint(-10, 10), self.y() + random.randint(-10, 10))
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        # Графика для квадратов
        self.scene = QGraphicsScene(self)
        self.view = QGraphicsView(self.scene, self)
        self.view.setGeometry(100, 100, 600, 600)

        # Создание и добавление 10 квадратов
        self.squares = [MovingSquare(random.randint(0, 550), random.randint(0, 550)) for _ in range(10)]
        for square in self.squares:
            self.scene.addItem(square)

        # Таймер для движения квадратов
        self.timer = QTimer(self)
        self.timer.timeout.connect(self.moveSquares)
